import { auth } from "@/auth";
import { prisma } from "@/lib/db";
import { redirect, notFound } from "next/navigation";
import Link from "next/link";
import { ymKey } from "@/lib/format";
import { getActiveHolder } from "@/lib/holder";
import {
  ForecastGridClient,
  type ForecastGridRow,
} from "@/components/forecast/ForecastGridClient";
import { ForecastSettingsButton } from "@/components/forecast/ForecastSettingsButton";
import { parseActualMonths } from "@/lib/forecast";

const MONTHS_LABEL = [
  "Gen",
  "Feb",
  "Mar",
  "Apr",
  "Mag",
  "Giu",
  "Lug",
  "Ago",
  "Set",
  "Ott",
  "Nov",
  "Dic",
];

const MODE_LABEL: Record<string, string> = {
  AVG_3M: "Media 3M",
  AVG_6M: "Media 6M",
  AVG_12M: "Media 12M",
  PREV_MONTH: "Mese prec.",
  SAME_MONTH_LY: "Stesso mese a.p.",
  MAX_3M: "Max 3M",
  ACTUAL_MONTH: "Consuntivo",
  MANUAL: "Manuale",
};

export default async function ForecastDetailPage(props: {
  params: Promise<{ id: string }>;
  searchParams: Promise<{ tipo?: "income" | "expense" | "all"; all?: string }>;
}) {
  const session = await auth();
  if (!session?.user) redirect("/login");
  const holder = await getActiveHolder(session.user.id!);
  const holderId = holder.id;

  const { id } = await props.params;
  const sp = await props.searchParams;
  const tipo = sp.tipo ?? "expense";
  const showAll = sp.all === "1";

  const forecast = await prisma.forecast.findFirst({
    where: { id, holderId },
    include: { budgets: true },
  });
  if (!forecast) notFound();

  const actualMonths = new Set(parseActualMonths(forecast.actualMonths));
  const year = forecast.year;

  const cats = await prisma.category.findMany({
    where: {
      holderId,
      ...(tipo !== "all" ? { type: tipo } : {}),
      ...(!showAll ? { hasBudget: true } : {}),
    },
    orderBy: [{ type: "asc" }, { name: "asc" }],
  });

  const allTx = await prisma.transaction.findMany({
    where: { holderId },
    select: { categoryId: true, date: true, amount: true },
  });
  const monthlyByCat: Record<string, Record<string, number>> = {};
  for (const t of allTx) {
    const ym = ymKey(t.date);
    monthlyByCat[t.categoryId] ??= {};
    monthlyByCat[t.categoryId][ym] = (monthlyByCat[t.categoryId][ym] || 0) + t.amount;
  }

  // Override forecast: mappa cat → mese → amount
  const fbByCat: Record<string, Record<number, number>> = {};
  for (const b of forecast.budgets) {
    fbByCat[b.categoryId] ??= {};
    fbByCat[b.categoryId][b.month] = b.amount;
  }

  const rows: ForecastGridRow[] = cats.map((c) => {
    const totals = monthlyByCat[c.id] ?? {};
    const months: ForecastGridRow["months"] = [];
    for (let m = 1; m <= 12; m++) {
      const period = `${year}-${String(m).padStart(2, "0")}`;
      const isActual = actualMonths.has(m);
      const actualAmt = totals[period] ?? 0;
      const auto = c.hasBudget
        ? computeBudgetForRow(c.budgetMode, period, totals, c.manualBudget)
        : 0;
      const override = fbByCat[c.id]?.[m];
      const budget = override ?? auto;
      const value = isActual ? actualAmt : budget;
      months.push({
        month: m,
        isActual,
        actual: actualAmt,
        auto,
        override: override ?? null,
        budget,
        value,
      });
    }
    const yearTotal = months.reduce((s, x) => s + x.value, 0);
    const overrideCount = months.filter((m) => m.override !== null).length;
    return {
      id: c.id,
      name: c.name,
      type: c.type as "income" | "expense",
      hasBudget: c.hasBudget,
      budgetMode: c.budgetMode,
      budgetModeLabel: MODE_LABEL[c.budgetMode] ?? c.budgetMode,
      manualBudget: c.manualBudget,
      monthlyTotals: totals,
      months,
      yearTotal,
      overrideCount,
    };
  });

  const totals = {
    perMonth: Array.from({ length: 12 }, (_, i) => ({
      isActual: actualMonths.has(i + 1),
      value: rows.reduce((s, r) => s + r.months[i].value, 0),
    })),
    yearTotal: rows.reduce((s, r) => s + r.yearTotal, 0),
  };

  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-3">
        <div className="min-w-0">
          <div className="text-xs text-sub">
            <Link href="/forecast" className="hover:text-ink">
              Forecast
            </Link>{" "}
            · {forecast.year}
          </div>
          <h1 className="text-xl font-semibold tracking-tight truncate">
            {forecast.name}
          </h1>
          {forecast.notes && (
            <div className="text-xs text-sub mt-0.5">{forecast.notes}</div>
          )}
        </div>
        <div className="flex gap-2 shrink-0">
          <ForecastSettingsButton
            id={forecast.id}
            initialName={forecast.name}
            initialYear={forecast.year}
            initialActualMonths={Array.from(actualMonths)}
            initialNotes={forecast.notes ?? ""}
          />
        </div>
      </div>

      <form action={`/forecast/${id}`} className="panel p-3 mb-3 flex flex-wrap gap-2 items-end">
        <label className="flex flex-col gap-0.5">
          <span className="text-[10px] uppercase tracking-wider text-sub font-semibold">
            Tipo
          </span>
          <select
            name="tipo"
            defaultValue={tipo}
            className="input !h-8 !py-0 min-w-[140px]"
          >
            <option value="expense">Solo uscite</option>
            <option value="income">Solo entrate</option>
            <option value="all">Tutte</option>
          </select>
        </label>
        <label className="flex items-center gap-1.5 mb-1 ml-2 cursor-pointer">
          <input
            type="checkbox"
            name="all"
            value="1"
            defaultChecked={showAll}
            className="accent-brand-500"
          />
          <span className="text-xs text-ink2">
            Mostra anche categorie senza budget attivo
          </span>
        </label>
        <button type="submit" className="btn !h-8 !text-xs">
          Applica filtri
        </button>
        <a href={`/forecast/${id}`} className="btn-ghost !h-8 !text-xs">
          Pulisci filtri
        </a>
        <div className="flex-1" />
        <div className="text-xs text-sub ml-3">
          <span className="inline-block w-3 h-3 rounded-sm bg-line2 mr-1 align-middle" />
          Consuntivo (read-only) ·{" "}
          <span className="inline-block w-3 h-3 rounded-sm bg-white border border-line ml-1 mr-1 align-middle" />
          Budget (modificabile)
        </div>
      </form>

      <ForecastGridClient
        forecastId={forecast.id}
        year={year}
        monthLabels={MONTHS_LABEL}
        rows={rows}
        totals={totals}
      />

      <div className="mt-4 panel p-4 text-xs text-sub leading-relaxed">
        <div className="font-medium text-ink mb-1">Come leggere il forecast</div>
        I mesi <strong>consuntivo</strong> mostrano la somma reale dei movimenti
        (sfondo grigio, non modificabili). I mesi <strong>budget</strong> mostrano
        l'importo pianificato (auto-calcolato dalla modalità della categoria, o
        l'override impostato in questo scenario). Click su una cella budget per
        modificarla — gli override sono salvati solo in questo scenario.
      </div>
    </div>
  );
}

function computeBudgetForRow(
  mode: string,
  period: string,
  monthlyTotals: Record<string, number>,
  manualAmount: number | null,
): number {
  if (mode === "MANUAL") return manualAmount ?? 0;
  if (mode === "ACTUAL_MONTH") return monthlyTotals[period] || 0;
  const shift = (yyyymm: string, back: number): string => {
    let [y, m] = yyyymm.split("-").map(Number);
    m -= back;
    while (m < 1) {
      m += 12;
      y -= 1;
    }
    return `${y}-${String(m).padStart(2, "0")}`;
  };
  const lastN = (n: number) =>
    Array.from({ length: n }, (_, i) => shift(period, i + 1));
  const avgOf = (months: string[]) => {
    const vals = months.map((ym) => monthlyTotals[ym]).filter((v) => v && v > 0);
    return vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 0;
  };
  let raw = 0;
  switch (mode) {
    case "AVG_3M":
      raw = avgOf(lastN(3));
      break;
    case "AVG_6M":
      raw = avgOf(lastN(6));
      break;
    case "AVG_12M":
      raw = avgOf(lastN(12));
      break;
    case "PREV_MONTH":
      raw = monthlyTotals[shift(period, 1)] || 0;
      break;
    case "SAME_MONTH_LY": {
      const [y, m] = period.split("-").map(Number);
      raw = monthlyTotals[`${y - 1}-${String(m).padStart(2, "0")}`] || 0;
      break;
    }
    case "MAX_3M":
      raw = Math.max(0, ...lastN(3).map((ym) => monthlyTotals[ym] || 0));
      break;
  }
  return Math.ceil(raw / 10) * 10;
}
